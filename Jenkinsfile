pipeline {
    ...
    stages {
        ...
        stage('get git files') {
            steps {
                    withCredentials(bindings: [usernamePassword(credentialsId: 'sct-git-credential', passwordVariable: 'PASWD', usernameVariable: 'USER')]) {
                        bat '''
                                cd tmp && 
                                git clone https://$USER:$PASWD@github.tools.sap/SCT/btp-data-model.git -b main
                            '''   
                    }                
